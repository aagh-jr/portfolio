...
const HeroSection = () => (
  <SectionWrap id="hero" style={{ padding: '128px 0 96px' }}>
    <Container>
      <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 28 }}>
        <Tag color="green">UX Research</Tag>
        <Tag color="green">Game Design</Tag>
        <Tag color="gray">UCSD ENG 100D · Winter 2025</Tag>
      </div>
      <h1 style={{ fontSize: 72, fontWeight: 700, lineHeight: 1.02, letterSpacing: '-0.035em', color: '#141414', marginBottom: 22, maxWidth: 820 }}>
        Energy Ave
      </h1>
      <p style={{ fontSize: 20, lineHeight: 1.65, color: '#666', maxWidth: 580, marginBottom: 52, fontWeight: 400 }}>
      ...
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginBottom: 64 }}>
        {[['Role','Researcher & Designer'],['Team','8 People'],['Duration','10 Weeks'],['Platform','Construct3 (Browser)'],['Client','PlanetFlip']].map(([l,v]) => (
          <div key={l} style={{ background: '#fff', borderRadius: 16, padding: '12px 20px', boxShadow: '0 2px 14px rgba(0,0,0,0.06)' }}>
            <div style={{ fontSize: 10, fontWeight: 700, color: '#3DAA74', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 4 }}>{l}</div>
            <div style={{ fontSize: 14, fontWeight: 600, color: '#141414' }}>{v}</div>
          </div>
        ))}
      </div>
      <Placeholder label="Hero screenshot — Energy Ave gameplay" height={440} note="Drop game screenshot / screen recording here — bedroom or bathroom scene" />
    </Container>
  </SectionWrap>
);
